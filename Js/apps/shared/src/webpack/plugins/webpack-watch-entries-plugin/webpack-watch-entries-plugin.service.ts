...
const IGNORE_TIME_ENTRY: any = 'ignore';
...
@CustomInjectable()
export class WebpackWatchEntriesPluginConfigService extends WebpackPluginBaseService {
    constructor() {
    ...
    createOptions(options?) {
        return this.mergeService.mergeOptions(
            ...
            } as WebpackWatchEntriesPluginOptions,
            options,
        );
    }
}
...
@CustomInjectable()
export class WebpackWatchEntriesPluginService extends WebpackPluginBaseService {
    constructor() {
        super(WebpackWatchEntriesPlugin);
    }
...
export class Extension {
    ...
    preserveOriginalOutput?: boolean;
}
...
export class PatternsCollection {
    ...
    output?: Output;
    patternsOptions?: any;
    excludeFromHtmlWebpackPlugin?: boolean;
    exportable?: boolean;
    ignoredFromWatch?: boolean;
...
class DbEntry {
    ...
    entryMap?: any;
    fileToChunkIdMap?: any;
    exportableChunkIds?: string[];
}
...
export class GlobalDb {
    ...
    getAllPatternsCollections(id): PatternsCollection[] {
        ...
        return patternsCollection.concat(
            ...dependsOn.map((dependentId) => {
                return this.getAllPatternsCollections(dependentId);
            }),
        );
    }

    getRefreshedData(id): DbEntry {
        ...
        patternsCollections.forEach(function (patternsCollection) {
            if (!patternsCollection.patterns) {
                patternsCollection = {
                    patterns: [patternsCollection as any],
                    output: {},
                    patternsOptions: {},
                } as PatternsCollection;
            }
            ...
            const {
                ...
                exportable,
                ignoredFromWatch: ignoredFromWatchArg,
                watchThePatternsOnly
            } = patternsCollection;

            if (ignoredFromWatchArg) {
                ignoredFromWatch = ignoredFromWatch.concat(
                    patterns.map((pattern) => pattern.split(path.sep).join(path.posix.sep)),
                );
            } else {
                patterns.forEach((pattern) => {
                    if (typeof pattern === 'string') {
                        ...
                        const filesToAdd: string[] = Object.values(entryFiles) ?? [];
...
export class WebpackWatchEntriesPlugin {
    ...
    constructor(private options: WebpackWatchEntriesPluginOptions) {
    ...
    apply(compiler: Compiler) {
        ...
        compiler.hooks.compilation.tap(WebpackWatchEntriesPlugin.name, (compilation) => {
            ...
            compilation.hooks.processAssets.tap(
                { name: WebpackWatchEntriesPlugin.name, stage: Compilation.PROCESS_ASSETS_STAGE_ADDITIONS },
                async (assets) => {
