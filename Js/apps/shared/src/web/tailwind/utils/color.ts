...
function buildPalette(hex) {
    ...
    return {
        'trans-light': tinycolor(lightest[50]).toRgbString().replace(')', ', 0.15)').replace('rgb(', 'rgba('),
        trans: tinycolor(lightest[50]).toRgbString().replace(')', ', 0.7)').replace('rgb(', 'rgba('),
        'trans-dark': tinycolor(hex).toRgbString().replace(')', ', 0.15)').replace('rgb(', 'rgba('),
        ...
        ...o(tinycolor.mix(baseDark, baseTriad[3], 15).saturate(80).lighten(65), 'a100'),
        ...o(tinycolor.mix(baseDark, baseTriad[3], 15).saturate(80).lighten(55), 'a200'),
        ...o(tinycolor.mix(baseDark, baseTriad[3], 15).saturate(100).lighten(45), 'a400'),
        ...o(tinycolor.mix(baseDark, baseTriad[3], 15).saturate(100).lighten(40), 'a700'),
    };
}

export function generate(colors) {
