...
commandObjs.forEach((commandObj) => {
    ...
    childs.forEach((target) => {
        final[`${commandObj.name}:${target.folderName}`] = `cd ${target.folderPath ?? target.folderName} && npm run {commandName}`.replace('{commandName}', commandObj.name);
        if (commandObj.includes.indexOf(target.folderName) !== -1) {
            scriptForAll += '"{commandName} -- {@}" '.replace('{commandName}', `${commandObj.name}:${target.folderName}`);
        }
    });
    scriptForAll += '--';
