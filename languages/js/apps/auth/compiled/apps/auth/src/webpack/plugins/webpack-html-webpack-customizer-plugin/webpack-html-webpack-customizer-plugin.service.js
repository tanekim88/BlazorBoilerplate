    ...
    apply(compiler) {
        compiler.hooks.compilation.tap(AuthWebpackHtmlWebpackCustomizerPlugin.name, (compilation) => {
            ...
            (data, cb) => {
                ...
                if (template?.endsWith(paths_1.authPaths.src.templates['Layout.cshtml'].toAbsolutePath())) {
                    data.headTags = data.headTags.filter((d) => {
                        const href = d.attributes?.href;
                        if (href && href.startsWith('/Shared_Material__')) {
                            return false;
                        }
                        return true;
                    });
                    data.bodyTags = data.bodyTags.filter((d) => {
                        ...
                        if (src && src.startsWith('/Shared_Material__')) {
                            return false;
                        }
                        return true;
                    });
                }
                if (template?.endsWith(paths_1.authPaths.src.templates['index.html'].toAbsolutePath())) {
                    data.headTags = data.headTags.filter((d) => {
                        const href = d.attributes?.href;
                        if (href && href.startsWith('/Shared_NativeMaterial__')) {
                            return false;
                        }
                        return true;
                    });
                    data.bodyTags = data.bodyTags.filter((d) => {
                        const src = d.attributes?.src;
                        if (src && src.startsWith('/Shared_NativeMaterial__')) {
                            return false;
                        }
                        return true;
                    });
