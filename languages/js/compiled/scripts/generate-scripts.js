...
const commandObjs = [
    ...
    //////////////////
    {
        ...
        command: "docker rm -f $(docker inspect --format='{{if ne \"dapr_\" (slice .Name 1 6) }}{{.Id}}{{end}}' $(docker ps -q))",
        includes: [rootProjName, ...appDirNames],
        static: true
    },
    {
    ...
    },
    {
        ...
        command: "docker inspect --format='{{if ne \"dapr_\" (slice .Name 1 6) }}{{.Id}}{{end}}' $(docker ps -q) | ForEach-Object -Process {docker rm -f $_}",
        includes: [rootProjName, ...appDirNames],
        static: true
    },
    /////////////
    ...
    },
    {
        ...
        command: 'cross-env NODE_OPTIONS="--max_old_space_size=4096 --loader ts-node/esm" --experimental-specifier-resolution=node vite --mode=production --config vite.prod.ts',
        includes: [...appDirNames],
    },
    {
        name: 'watch:vite',
        command: 'cross-env NODE_OPTIONS="--max_old_space_size=4096 --loader ts-node/esm" --experimental-specifier-resolution=node vite --mode=development --watch --config vite.dev.ts',
        includes: [...appDirNames],
    },
    {
        name: 'watch:prod:vite',
        command: 'cross-env NODE_OPTIONS="--max_old_space_size=4096 --loader ts-node/esm" --experimental-specifier-resolution=node vite --mode=production --watch --config vite.prod.ts',
        includes: [...appDirNames],
    },
    {
        name: 'start:pwa',
    ...
    if (!commandObj.static) {
        childs.forEach((child) => {
            if (commandObj.includes.indexOf(child.folderName) !== -1) {
                final[`${commandObj.name}:${child.folderName}`] = `cd ${child.relativeFolderPath ?? child.folderName} && npm run {commandName}`.replace('{commandName}', commandObj.name);
                scriptForAll += '"{commandName} -- {@}" '.replace('{commandName}', `${commandObj.name}:${child.folderName}`);
            }
        });
        scriptForAll += '--';
