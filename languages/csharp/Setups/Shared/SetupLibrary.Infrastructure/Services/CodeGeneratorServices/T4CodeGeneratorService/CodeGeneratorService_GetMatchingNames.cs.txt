...
namespace SetupLibrary.Infrastructure.Services.CodeGeneratorServices
{
    public partial class CodeGeneratorService
    {
            ...
            List<TemplateTokenInfo> toReturn = null)
        {
            ...
            {
                string.Join(separator: codeType.GetPropertyPathSeparator(isPath: isPath), values: parentPropertyPaths)
            };
            ...
            {
                var tuples =
                    codeType.GetCasedNameAndCasedValueTuples_IncludingBothSingularAndPlural(name: lastPath, value: "");
            ...
            var matchFound = candidates.Any(predicate: candidate =>
            {
                ...
                found = listOfCombinedPaths.FirstOrDefault(predicate: x =>
                    candidate.StartsWith(value: x + codeType.GetPropertyPathSeparator(isPath: isPath)));
            ...
            if (objectType.IsEnumerableType() && objectType.IsGenericType)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            parentName,
                            $@"(?<Matched>\d+)(?={codeType.GetPropertyPathSeparator(isPath: isPath)}[a-zA-Z\d]+)"
                        }, isPath: isPath);
