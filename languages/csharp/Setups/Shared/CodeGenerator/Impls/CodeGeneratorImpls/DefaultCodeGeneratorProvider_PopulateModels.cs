...
namespace CodeGenerator.Impls.CodeGeneratorImpls
{
    public partial class DefaultCodeGeneratorProvider : ICodeGeneratorProvider
    {
        public async Task PopulateModels(TemplateData data)
        {
            ...
            if (data.Context.BoundedContext.Models.Count() != 0) return;
            ...
            foreach (var project in projects)
            {
               ...
               !p.IsShared);
                ...
                else
                {
                    modelTypes?.ForEach(action: modelType =>
                    {
                        ...
                        if (groups.Count() == 0 || groups[index: 0].Name != "__Models_Groups_00_Name__")
                        {
                            ...
                            {
                                Name = modelType.Name,
                                PluralName = modelType.Name.Pluralize(),
                                IsValueObject = isValueObject,
                                BaseName = baseName,
                ...
                data.Context.BoundedContext.Models.AddRange(collection: models);
                data.Context.BoundedContext.Entities.AddRange(collection: entities);
            }
        }
