    ...
    public partial class DefaultCodeGeneratorProvider : ICodeGeneratorProvider<TemplateData>
    {
        public void PopulateProjects(TemplateData data)
        {
            foreach (var project in data.Projects)
            {
                ...
                if (data.Context.BoundedContext.Services is not null) {
                    project.Services = data.Context.BoundedContext.Services.Where(
                        predicate: service => service.Project.Name == project.Name)
                        .ToList();
                }

                if (data.Context.BoundedContext.Models is not null)
                    project.Models = data.Context.BoundedContext.Models.Where(
                        predicate: model => model.Project.Name == project.Name).ToList();

                if (data.Context.BoundedContext.Entities is not null)
                    project.Entities = data.Context.BoundedContext.Entities.Where(
