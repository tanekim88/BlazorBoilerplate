...
namespace CodeGenerator.Services.CodeGeneratorServices
{
    ...
    public partial class CodeGeneratorService<TData, TFile> 
    {
        ...
        public async Task<ProcessRefOutput> ProcessRefAsync(
            TemplateFile file,
            TData data,
            Dictionary<string, TemplateProject> projectNameToProjectDic
            )
        {
            ...
            if (!commandContent.Success)
            {
                commandContent = await GetFirstCommandContentAsync(inputString: file.Content, command: TemplateCommand.PartialRefProject,
                    codeType: file.CodeType);
            ...
            {
                var evaluateAsyncOutput = await _evalService.EvaluateAsync<TData, string>(commandContent.Payload, data);
