...
using Humanizer;
using CodeGenerator.Models.Dictionaries;
...
namespace CodeGenerator.Models
{
    ...
    public class TemplateCodeType
    {
        ...
        public string GetPropertySeparator(bool isPath)
        {
            if (isPath)
        ...
        {
            string toReturn = string.Join(separator: GetPropertySeparator(isPath: isPath),
        ...
        )
        {
            ...
            var matches = GetCasedNameAndCasedValueTuples_IncludingBothSingularAndPlural(name: name, value: "", sep)
                .Select(selector: casedName =>
                {
                    var nameRegex =
                        @$"({GetPropertySeparator(isPath: isPath)}(?<Name>{casedName.Name})){(isNameOptional ? "?" : "")}";
                    var argRegex =
                        @$"({GetPropertySeparator(isPath: isPath)}(?<ArgName>{argName})){(isArgNameOptional ? "?" : "")}";
        ...
        public List<(string Name, string Value)> GetCasedNameAndCasedValueTuples_IncludingBothSingularAndPlural(
            string name, string value, string sep)
        {
            var pascalName = ChangeNameCase(name: name, fun: input => input.Pascalize(), sep);
            var pascalPluralName = ChangeNameCase(name: pascalName, fun: input => input + sep + "Plural", sep);
                ...
                {(pascalName, value, false), (pascalPluralName, pluralValue, true)}.SelectMany(selector: arg =>
            {
                ...
                var camelName = ChangeNameCase(name: name, fun: input => input.Camelize(), sep);
                ...
                var toReturn = new List<(string, string)>
                {
                    ...
                    (name + sep + CamelCasePostfix, ChangeCase(name: value, fun: input => input.Pascalize())),
                    (name + sep + PascalCasePostfix, ChangeCase(name: value, fun: input => input.Pascalize())),
                    (name + sep + KebabCasePostfix, ChangeCase(name: value, fun: input => input.Kebaberize())),
                    (name + sep + HumanCasePostfix, ChangeCase(name: value, fun: input => input.Humanize())),
                    (name + sep + HumanTitleCasePostfix,
        ...
        public List<(string Name, string Value)> GetCasedNameAndCasedValueTuples(string name, string value, string speartor)
        {
            var pascalName = ChangeNameCase(name: name, fun: input => input.Pluralize(), speartor);
            var pascalPluralName = ChangeNameCase(name: pascalName, fun: input => input.Pluralize(), speartor);
            ...
            var toReturn = new List<(string, string)> { (pascalName, value) }.SelectMany(selector: arg =>
              {
                  var (name, value) = arg;
                  var camelName = ChangeNameCase(name: name, fun: input => input.Camelize(), speartor);
        ...
        public string ChangeNameCase(string name, Func<string, string> fun, string seperator)
        {
            if (name.Contains(value: seperator))
            {
                var splittedBy_ = name.Split(separator: seperator);
                var last = splittedBy_.Last();
                var rest = splittedBy_.SkipLast(count: 1);
                var restJoined = string.Join(separator: seperator, values: rest);
                var casedLast = fun(arg: last);
                var toReturn = string.Join(separator: seperator, restJoined, casedLast);
                return toReturn;
            }
