...
namespace Auth.Server.Areas.Identity.Pages.Account.Manage
{
    public class EnableAuthenticatorModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!is2faTokenValid)
            {
                ModelState.AddModelError(key: "Input.Code", errorMessage: "Verification code is invalid.");
                await LoadSharedKeyAndQrCodeUriAsync(user: user);
                return Page();
            }
            ...
            _logger.LogInformation(message: "User with ID '{UserId}' has enabled 2FA with an authenticator app.",
                userId);
        ...
        private string FormatKey(string unformattedKey)
        {
            ...
            {
                result.Append(value: unformattedKey.Substring(startIndex: currentPosition, length: 4))
                    .Append(value: " ");
                currentPosition += 4;
            }

            if (currentPosition < unformattedKey.Length)
                result.Append(value: unformattedKey.Substring(startIndex: currentPosition));
        ...
        private string GenerateQrCodeUri(string email, string unformattedKey)
        {
            return string.Format(
                ...
                arg0: _urlEncoder.Encode(value: "Auth.Server"),
